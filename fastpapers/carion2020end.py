...
__all__ = ['coco_vocab', 'clip_remove_empty', 'bb_pad', 'box_cxcywh_to_xyxy', 'box_xyxy_to_cxcywh', 'TensorBBoxWH',
...
# Cell
@Transform
def ToWH(x:TensorBBox): return TensorBBoxWH(box_xyxy_to_cxcywh(x*0.5+0.5))

# Cell
@Transform
def ToXYXY(x:TensorBBoxWH)->None:
    return TensorBBox(box_cxcywh_to_xyxy(x)*2-1)

# Cell
class ToTL(Transform):
    def encodes(self, x:TensorBBoxWH)->None: return TensorBBoxTL(box_cxcywh_to_xyxy(x))
    def encodes(self, x:TensorBBox)->None: return TensorBBoxTL((x+1)/2)
...
class DETRLoss(nn.Module):
    def __init__(self, classw=1, boxw=1, giouw=1, n_queries=100, th=0.7, eos_coef=0.1, n_classes=92): #, scores=False
        super().__init__()
        store_attr()
    ...
    def box_loss(self, output_boxes, target_boxes, indices):
        ...
        output_boxes_ind = TensorBBoxWH(torch.cat(output_boxes_ind))
        target_boxes_ind = TensorBBoxWH(torch.cat(target_boxes_ind))
    ...
    def box_cost(self, output_boxes, target_boxes):
        output_boxes, target_boxes = ToWH((output_boxes, target_boxes))
        return torch.cdist(output_boxes, target_boxes, p=1) # [bs, nq, nq]
    ...
    def decodes(self, x, pad=True):
        ...
        #if self.scores:
        #    return pred_boxes, pred_ids, max_probs
        #else:
...
class DETR(nn.Module):
    ...
    def forward(self, x):
        ...
        if aux_outputs:
            for o in aux_outputs: o['pred_boxes'] = TensorBBoxWH(o['pred_boxes'])
        return TensorBBoxWH(pred_boxes), pred_logits, aux_outputs
